...
public class LeakExample {
	...
	public void slowlyLeakingVector(int iter, int count) {
		for (int i = 0; i < iter; i++) {
			...
			for (int n = count - 1; n > 0; n--) { // Oops, it should be n>=0
	...
	public void noLeak(int size) {
		...
		// Though highest memory allocation happens in this function, but all
		// these
		// objects get garbage collected at the end of this method, so no leak.
	}
